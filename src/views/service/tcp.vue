...
import { serviceDetail, serviceTCPCreate, serviceTCPUpdate } from '@/api/service'
...
export default {
  ...
  methods: {
    ...
    onsubmitHandle() {
      ...
      if (this.isEdit) {
        serviceTCPUpdate(this.form.id, query).then(response => {
        ...
        serviceTCPCreate(query).then(response => {
          this.submitDisableButton = false
          this.$notify({
            message: '添加成功！',
            type: 'success',
